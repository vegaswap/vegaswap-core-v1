...
contract VestingController is OwnableUpgradeable, PausableUpgradeable {
  ...
  modifier pastCliffTime(address addr) {
    ...
    require(block.timestamp > vestingSchedule.cliffTime, "VESTING: PAST_CLIFF_TIME");
    _;
  }
  ...
  //        pastCliffTime(vestingSchedule.registeredAddress) // dont throw error
  {
    ...
    if (vestingSchedule.totalWithdrawnAmount >= vestingSchedule.totalAmount) {
      return vestingSchedule.totalWithdrawnAmount;
    }
    ...
    vestingSchedule.totalWithdrawnAmount = vestingSchedule.totalWithdrawnAmount + withdrawableAmount;
