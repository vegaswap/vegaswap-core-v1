...
import { duration, getContext, increaseBlockTime, latest, toSolNumber, ZERO_ADDRESS } from "../utils";
...
async function main() {
  ...
  const vestingLog = ({ address, cliff, period, amount }) => {
    ...
    console.log(
      `Register ${address} with cliff: ${cliff}, release gradually in ${_period}, total amount: ${amount} successfully.`,
    );
    return {
